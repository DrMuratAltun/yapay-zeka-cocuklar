...
import { BOLUM_META, okuBolumIlerleme, type BolumMeta } from "@/data/bolumler";
...
/** Son 7 günün aktivite sayıları — localStorage'daki progress-* kayıtlarından (Pzt..Paz). */
function hesaplaHaftalikAktivite(): number[] {
  const gunler = [0, 0, 0, 0, 0, 0, 0];
  try {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      try {
        ...
        const t = new Date(kayit.completedAt);
        const farkGun = Math.floor((simdi.getTime() - t.getTime()) / 86400000);
        if (farkGun >= 0 && farkGun < 7) {
          const haftaGunu = (t.getDay() + 6) % 7; // Pzt=0 ... Paz=6
          gunler[haftaGunu] += 1;
        }
      } catch {}
    }
...
function BolumIzleCard({ bolum, ilerlemePct }: { bolum: BolumMeta; ilerlemePct: number }) {
  return (
    <Link
...
function IlerlemeBar({ bolum, ilerlemePct, passed }: { bolum: BolumMeta; ilerlemePct: number; passed: boolean }) {
  return (
    <Link
      href={`/bolumler/${bolum.no}`}
...
export default function OgrenciDashboard() {
  ...
  const [aktivite7Gun, setAktivite7Gun] = useState<number[]>([0, 0, 0, 0, 0, 0, 0]);
  ...
  useEffect(() => {
    ...
    let konuToplam = 0;
    for (const b of BOLUM_META) {
      const ozet = okuBolumIlerleme(b.no);
      out[b.no] = ozet.yuzde;
      konuToplam += ozet.tamam;
    }
    setBolumIlerlemeleri(out);
    setTamamlananKonu(konuToplam);
    setAktivite7Gun(hesaplaHaftalikAktivite());
  }, []);
  ...
  const assignedBolumler = studentModules.length > 0
    ? BOLUM_META.filter((b) => moduleMap.has(b.no))
    : BOLUM_META;
  ...
  // XP: quiz skorları + tamamlanan her konu için 5 puan
  const xpToplam =
    studentModules.reduce((acc, m) => acc + (m.quizResult?.score ?? 0), 0) + tamamlananKonu * 5;
  ...
  // Arama: bölüm listesini başlığa göre filtrele
  const filtreliIlerlemeler = useMemo(() => {
    const q = arama.trim().toLowerCase();
    if (!q) return ilerlemeler;
    return ilerlemeler.filter(
      ({ b }) =>
        b.baslik.toLowerCase().includes(q) ||
        b.altBaslik.toLowerCase().includes(q) ||
        String(b.no) === q
    );
  }, [ilerlemeler, arama]);
  ...
  return (
    <div className="flex min-h-screen gap-6 p-4 sm:p-6 lg:p-8">
      ...
      <div className="flex-1 min-w-0 space-y-6">
        {/* Arama — bölüm listesini filtreler */}
        <div className="relative">
          <input
            type="search"
            value={arama}
            onChange={(e) => setArama(e.target.value)}
            placeholder="Bölüm ara... (örn. veri, etik, kodlama)"
        ...
        <section>
          ...
          {filtreliIlerlemeler.length === 0 ? (
            <p className="rounded-xl border border-dashed border-[var(--color-border)] p-6 text-center text-sm text-muted-foreground">
              &quot;{arama}&quot; ile eşleşen bölüm yok. Farklı bir kelime dene 🔍
            </p>
          ) : (
            <div className="grid gap-3 sm:grid-cols-2">
              {filtreliIlerlemeler.map(({ b, p, passed }) => (
      ...
      <aside className="hidden xl:flex xl:w-72 shrink-0 flex-col gap-5">
        ...
        <div className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-bg)] p-5">
          <div className="flex items-center justify-between">
            ...
            <Link href="/ogrenci/profil" className="text-xs font-semibold text-violet-600 hover:underline dark:text-violet-400">
              Düzenle
            </Link>
          </div>
